...
public class SoundAdapter extends ArrayAdapter<SoundInfo> implements
	...
	public SoundAdapter(final Context context, int textViewResourceId,
			ArrayList<SoundInfo> items, boolean showDetails) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = View.inflate(context,
					R.layout.fragment_sound_soundlist_item, null);

			holder = new ViewHolder();

			holder.playButton = (ImageButton) convertView
					.findViewById(R.id.btn_sound_play);
			holder.pauseButton = (ImageButton) convertView
					.findViewById(R.id.btn_sound_pause);

			holder.playButton.setVisibility(Button.VISIBLE);
			holder.pauseButton.setVisibility(Button.GONE);

			holder.checkbox = (CheckBox) convertView
					.findViewById(R.id.sound_checkbox);

			holder.titleTextView = (TextView) convertView
					.findViewById(R.id.sound_title);
			holder.timeSeperatorTextView = (TextView) convertView
					.findViewById(R.id.sound_time_seperator);
			holder.timeDurationTextView = (TextView) convertView
					.findViewById(R.id.sound_duration);
			holder.soundFileSizeTextView = (TextView) convertView
					.findViewById(R.id.sound_size);
		...
		if (soundInfo != null) {
			...
			try {
				...
				if (hours == 0) {
					holder.timeDurationTextView.setText(String.format(
				...
				if (currentPlayingPosition == Constants.NO_POSITION) {
					...
					elapsedMilliSeconds = SystemClock.elapsedRealtime()
							- currentPlayingBase;
				}
				...
				if (soundInfo.isPlaying) {
					...
					holder.timeSeperatorTextView
							.setVisibility(TextView.VISIBLE);
					holder.timePlayedChronometer
							.setVisibility(Chronometer.VISIBLE);

					if (currentPlayingPosition == Constants.NO_POSITION) {
						startPlayingSound(holder.timePlayedChronometer,
								position);
					} else if ((position == currentPlayingPosition)
							&& (elapsedMilliSeconds > (milliseconds - 1000))) {
						stopPlayingSound(soundInfo,
								holder.timePlayedChronometer);
					} else {
						continuePlayingSound(holder.timePlayedChronometer,
					...
					holder.timePlayedChronometer
							.setVisibility(Chronometer.GONE);
				...
				if (showDetails) {
					holder.soundFileSizeTextView.setText(getContext()
							.getString(R.string.size)
							+ " "
							+ UtilFile.getSizeAsString(new File(soundInfo
									.getAbsolutePath())));
