...
public class SpritesListFragment extends SherlockListFragment implements
		OnSpriteCheckedListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			spriteToEdit = (Sprite) savedInstanceState
					.get(BUNDLE_ARGUMENTS_SPRITE_TO_EDIT);
		}
	...
	public void onResume() {
		...
		IntentFilter intentFilterSpriteRenamed = new IntentFilter(
				ScriptActivity.ACTION_SPRITE_RENAMED);
		getActivity().registerReceiver(spriteRenamedReceiver,
				intentFilterSpriteRenamed);

		IntentFilter intentFilterSpriteListChanged = new IntentFilter(
				ScriptActivity.ACTION_SPRITES_LIST_CHANGED);
		getActivity().registerReceiver(spritesListChangedReceiver,
				intentFilterSpriteListChanged);

		IntentFilter intentFilterSpriteListInit = new IntentFilter(
				ScriptActivity.ACTION_SPRITES_LIST_INIT);
		getActivity().registerReceiver(spritesListInitReceiver,
				intentFilterSpriteListInit);
	...
	public void handleCheckBoxClick(View view) {
		...
		getListView().setItemChecked(
				position,
				((CheckBox) view.findViewById(R.id.sprite_checkbox))
						.isChecked());
	}

	public void showRenameDialog() {
		RenameSpriteDialog dialog = RenameSpriteDialog.newInstance(spriteToEdit
				.getName());
		dialog.show(getFragmentManager(),
				RenameSpriteDialog.DIALOG_FRAGMENT_TAG);
	}
	...
	private class SpriteRenamedReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(ScriptActivity.ACTION_SPRITE_RENAMED)) {
				String newSpriteName = intent.getExtras().getString(
						RenameSpriteDialog.EXTRA_NEW_SPRITE_NAME);
	...
	private ActionMode.Callback deleteModeCallBack = new ActionMode.Callback() {
		...
		public void onDestroyActionMode(ActionMode mode) {
			...
			while (iterator.hasNext()) {
				...
				spriteToEdit = (Sprite) getListView().getItemAtPosition(
						position - numDeleted);
				deleteSprite();
				numDeleted++;
			}
	...
	private void initListeners() {
		spriteList = (ArrayList<Sprite>) ProjectManager.getInstance()
				.getCurrentProject().getSpriteList();
		spriteAdapter = new SpriteAdapter(getActivity(),
				R.layout.activity_project_spritelist_item, R.id.sprite_title,
				spriteList);
		...
		getListView().setOnItemClickListener(
				new ListView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						ProjectManager.getInstance().setCurrentSprite(
								spriteAdapter.getItem(position));
		...
		getListView().setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				// fine
				if (ProjectManager.getInstance().getCurrentProject()
						.getSpriteList().indexOf(spriteToEdit) == 0) {
					return true;
				} else {
					return false;
				}
	...
	private void deleteSpriteFiles() {
		...
		for (LookData currentLookData : lookDataList) {
			StorageHandler.getInstance().deleteFile(
					currentLookData.getAbsolutePath());
		}

		for (SoundInfo currentSoundInfo : soundInfoList) {
			StorageHandler.getInstance().deleteFile(
					currentSoundInfo.getAbsolutePath());
		}
	}
