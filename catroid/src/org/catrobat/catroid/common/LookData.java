...
public class LookData implements Serializable {
	...
	public Pixmap getPixmap() {
		if (pixmap == null) {
			if (Utils.isLoadingFromAssetsNecessary()) {
				pixmap = new Pixmap(Gdx.files.internal(getAbsolutePath()));// No
																			// absolute
																			// path
																			// to
																			// assets
	...
	public Pixmap getOriginalPixmap() {
		if (originalPixmap == null) {
			if (Utils.isLoadingFromAssetsNecessary()) {
				originalPixmap = new Pixmap(
						Gdx.files.internal(getAbsolutePath()));// No absolute
																// path to
																// assets
																// possible.
			} else {
				originalPixmap = new Pixmap(
						Gdx.files.absolute(getAbsolutePath()));
			}
		}
	...
	public Bitmap getThumbnailBitmap() {
		if (thumbnailBitmap == null) {
			thumbnailBitmap = ImageEditing
					.getScaledBitmapFromPath(getAbsolutePath(),
	...
	public Bitmap getLookBitmap() {
		if (lookBitmap == null) {
			...
			// try {
			// InputStream istr =
			// LiveWallpaper.getContext().getAssets().open(getPath());
