...
public class SetLookBrick implements Brick {
	...
	@Override
	public void execute() {
		if (look != null && sprite != null
				&& sprite.getLookDataList().contains(look)) {
	...
	public View getView(final Context context, int brickId, BaseAdapter adapter) {
		...
		Spinner lookbrickSpinner = (Spinner) view
				.findViewById(R.id.setlook_spinner);
		...
		if (sprite.getLookDataList().contains(look)) {
			lookbrickSpinner.setSelection(sprite.getLookDataList()
					.indexOf(look) + 1, true);
		} else {
		...
		if (sprite.getName().equals(context.getString(R.string.background))) {
			TextView textView = (TextView) view
					.findViewById(R.id.brick_set_look_prototype_text_view);
	...
	private ArrayAdapter<?> createLookAdapter(Context context) {
		ArrayAdapter<LookData> arrayAdapter = new ArrayAdapter<LookData>(
				context, android.R.layout.simple_spinner_item);
		arrayAdapter
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		LookData dummyLookData = new LookData();
		dummyLookData.setLookName(context
	...
	public View getPrototypeView(Context context) {
		View prototypeView = View.inflate(context, R.layout.brick_set_look,
				null);
		if (sprite.getName().equals(context.getString(R.string.background))) {
	...
	public void executeLiveWallpaper() {
		...
		if (wallpaperLook != null && look != null) {
			wallpaperLook.setLook(look);
		} else {
			new WallpaperLook(sprite, look);
