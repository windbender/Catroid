...
public class PointInDirectionBrick implements Brick, View.OnClickListener {
	...
	public View getView(final Context context, int brickId, BaseAdapter adapter) {
		...
		TextView setAngleTextView = (TextView) view
				.findViewById(R.id.brick_point_in_direction_prototype_text_view);
		setAngleEditText = (EditText) view
	...
	public void onClick(View view) {
		...
		editDialog.show(activity.getSupportFragmentManager(),
				"dialog_point_in_direction_brick");
	}
	...
	private class EditPointInDirectionBrickDialog extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			View dialogView = LayoutInflater.from(getActivity()).inflate(
					R.layout.dialog_text_dialog, null);
			input = (EditText) dialogView
					.findViewById(R.id.dialog_text_EditText);
			...
			Dialog dialog = new AlertDialog.Builder(getActivity())
					.setView(dialogView)
					.setTitle(
							R.string.brick_point_in_direction_choose_direction)
					.setSingleChoiceItems(R.array.point_in_direction_strings,
							-1, new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog,
										int item) {

									String[] pointInDirectionValues = getResources()
											.getStringArray(
			...
			dialog.setOnShowListener(new OnShowListener() {
				...
				public void onShow(DialogInterface dialog) {
					Button buttonPositive = ((AlertDialog) getDialog())
		...
		protected boolean handleOkButton() {
			try {
				...
				Toast.makeText(getActivity(), R.string.error_no_number_entered,
		...
		private void initTextChangedListener() {
			final Button buttonPositive = ((AlertDialog) getDialog())
					.getButton(DialogInterface.BUTTON_POSITIVE);
	...
	public void executeLiveWallpaper() {
		...
		if (!(wallpaperLook.getRotation() == newRotation)) {
			wallpaperLook.setRotation((float) (-degrees + 90.0f));
		}
